...
class ASTRA(nn.Module):
    ...
    def __init__(
        ...
        if self.resnet:
            ...
            # 1 PFFN layers
            self.Rfeat_dim = [512]
            self.resnet_LL = nn.ModuleList([Rfeat_module(self.Rfeat_dim[i], model_cfg['dim'], drop = model_cfg['dropout']) for i in range(len(self.Rfeat_dim))])
            self.encTR = nn.Parameter(torch.rand(self.chunk_size, model_cfg['dim']))
            self.encFR = nn.Parameter(torch.rand(len(self.Rfeat_dim), model_cfg['dim']))
        ...
        #Features augmentation (for both audio + resnet)
        if model_cfg['feature_augmentation']:
            if self.resnet:
                self.temporal_dropR = temporal_dropM(p = model_cfg['temporal_drop_p'])
            if self.audio:
        ...
        if model_cfg['mixup']:
            ...
            self.register_buffer('featRQ', torch.zeros(model_cfg['num_classes'] + 1, model_cfg['mixup_nqueue'], self.chunk_size, 512))
            if self.audio:
    ...
    def forward(self, featsR = None, featsA = None, labels = None, labelsD = None, inference = False):
        ...
        if self.model_cfg['mixup'] & (not inference):
            ...
            featsR, featsA, labels, labelsD = self.do_mixup(featsR, self.featRQ, featsA, self.featAQ, labels, self.labelQ, labelsD, self.labelDQ)
            ...
            for i in range(self.model_cfg['num_classes']+1):
                ...
                if len(aux) >= self.model_cfg['mixup_nqueue']:
                    ...
                    if self.resnet:
                        self.featRQ[i, :] = xR[idx].clone().detach()
                    if self.audio:
                    ...
                    if self.resnet:
                        self.featRQ[i, idx1] = xR[idx].clone().detach()
                    if self.audio:
        ...
        if self.resnet:
            ...
            if self.model_cfg['feature_augmentation'] & (not inference):
                featsR = self.temporal_dropR(featsR)
                featsR = self.random_switch(featsR)
            #PFFN            
            featsR = [self.resnet_LL[i](featsR[:, :, int(torch.tensor(self.Rfeat_dim[:i]).sum()):int(torch.tensor(self.Rfeat_dim[:i+1]).sum())]) for i in range(len(self.Rfeat_dim))]
            featsR = torch.stack(featsR) #1 x b x cs x d
            #Positional Encoding
            l, b, cs, d = featsR.shape
            featsR += self.encTR.expand(l, b, -1, -1)
            featsR = rearrange(featsR, 'l b cs d -> b cs l d') + self.encFR.expand(b, cs, -1, -1)
            featsR = rearrange(featsR, 'b cs l d -> b (cs l) d')
        ...
        for i, mod in enumerate(self.Tencoder.layers):
            ...
            #hierarchical attention layers (i.e. split > 1 - split in segments)
                ...
                x = rearrange(featsR[:, :(splits[i]-1) * (nR // splits[i])], 'b (ns ls) d -> (b ns) ls d', ns = splits[i] - 1) #split in segments 
                x_aux = featsR[:, (splits[i]-1) * (nR // splits[i]):] #residual segment
                ...
                xR = torch.cat((rearrange(x[:, :(nR // splits[i])], '(b ns) ls d -> b (ns ls) d', ns = splits[i] - 1), x_aux[:, :(nR - (nR // splits[i] * (splits[i] - 1)))]), dim = 1)
                featsR = xR

                if self.audio:
                    xA = torch.cat((rearrange(x[:, (nR // splits[i]):(nR // splits[i]) + (nA // splits[i])], '(b ns) ls d -> b (ns ls) d', ns = splits[i] - 1), x_aux[:, (nR - (nR // splits[i] * (splits[i] - 1))):(nR - (nR // splits[i] * (splits[i] - 1))) + (nA - (nA // splits[i] * (splits[i] - 1)))]), dim = 1)
                    featsA = xA
...
class mixupYolo(torch.nn.Module):
    ...
    def forward(self, featR, featRQ, featA, featAQ, labels, labelsQ, labelsD, labelsDQ):
        #len of batch
        b = len(labels)
        ...
        #Mixture
        if featR != None:
            featR = featR * lamb + (1-lamb) * featRQ[idxa, idxnq]
        if featA != None:
        ...
        return featR, featA, labels, labelsD
